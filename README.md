...
This library is designed for **Next.js** projects using **Tailwind CSS**. The following must already be installed and configured in your project before using this component.
...
| `next`                  | `>=13`   |                                            |
| `tailwindcss`           | `>=3`    |                                            |
| `@tanstack/react-table` | `>=8`    | Used internally for table state management |
| `lucide-react`          | `>=0.3`  | Used for icons inside the component        |
...
After installation, two things need to be configured in every project that uses this library.

### 1. `next.config.js`

Add webpack aliases to prevent duplicate React instances and add the library package to `transpilePackages`:
...
const nextConfig = {
  ...
  transpilePackages: ['@ankamala/table'],
};

export default nextConfig;
```

> **Why?** When the library is installed from Git, Node.js can resolve a separate copy of React inside the package. The webpack aliases force all code to share a single instance, preventing "invalid hook call" errors.
...
> **Why?** Tailwind only includes CSS classes it finds at build time. Without this line, any Tailwind class used inside the component will be stripped from the production bundle and the UI will break.

---

## Quick Start

Once the setup above is complete, import and use the component directly:
